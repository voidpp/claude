  ...
  return (
    <tr>
      ...
      <IfComp cond={settings.columns.memory}>
        <td style={{ textAlign: "right" }}>{info.status ? `${info.status.memory.percent.toFixed(1)}%` : null}</td>
      </IfComp>
        ...
        <table>
          ...
          <tbody>
            {Object.values(settings.servers)
              ...
              .map(server => (
                <StatusRow config={server} settings={settings} key={server.name} widgetSize={rndProps.size} />
              ))}
          </tbody>
        </table>
      </Box>
